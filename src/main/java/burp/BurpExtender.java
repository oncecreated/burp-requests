...
public class BurpExtender implements IBurpExtender, IContextMenuFactory, ClipboardOwner {
    ...
    private static OutputStream stdError;

    private boolean insertHostVariable = false;
    private final static String insertHostVariableKey = "insertHostVariable";
    private boolean addHeaders = true;
    private final static String addHeadersKey = "addHeaders";
    private boolean addCookie = true;
    private final static String addCookieKey = "addCookie";
    private boolean addImport = true;
    private final static String addImportKey = "addImport";
    private boolean alwaysSessionPrefix = false;
    private final static String alwaysSessionPrefixKey = "alwaysSessionPrefix";
    ...
    public void registerExtenderCallbacks(IBurpExtenderCallbacks callbacks) {
        try {
            String content = new String(Files.readAllBytes(Paths.get("requests.settings.json")), StandardCharsets.UTF_8);
    ...
    private void parseSettings(String content) {
        Json settings = Json.read(content);
        insertHostVariable = settings.at(insertHostVariableKey).asBoolean();
        addHeaders = settings.at(addHeadersKey).asBoolean();
        addCookie = settings.at(addCookieKey).asBoolean();
        addImport = settings.at(addImportKey).asBoolean();
        alwaysSessionPrefix = settings.at(alwaysSessionPrefixKey).asBoolean();
    }
    ...
    private URL replaceHost(URL original, String host) {
        try {
            return new URL(original.getProtocol(), host,
                    original.getPort(), original.getFile());
    ...
    private void copyMessages(IHttpRequestResponse[] messages, boolean withSessionObject) {
        ...
        for (IHttpRequestResponse message : messages) {
            ...
            if (insertHostVariable) {
                String randomHost = UUID.randomUUID().toString();
                url = replaceHost(url, randomHost);
                urlValue = escapeQuotes(url.toString()).replaceFirst(randomHost, "\" + host + \"");
            } else {
                urlValue = escapeQuotes(url.toString());
            }
            ...
            if (addCookie) {
                cookiesExist = processCookies(prefix, py, headers);
            }
